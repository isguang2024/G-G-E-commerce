...
- menu space：回答“当前 APP 应展示哪组导航壳与菜单树”。
...
- 只记录“已经做了什么、验证到了什么、下一步是什么”，不重复协议细节。
...
flowchart LR
  ...
  admin --> runtimeApi["运行时导航 / 页面 / menu space"]
  remote --> runtimeApi
...
| `menu space` | APP 内导航与首页的运行空间 | `menu_spaces` |
...
| `runtime navigation` | 目标 APP + space 下的导航清单与默认首页解析结果 | `/runtime/navigation` |
