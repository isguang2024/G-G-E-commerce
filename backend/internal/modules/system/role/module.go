...
func (m *RoleModule) RegisterRoutes(rg *gin.RouterGroup) {
	...
	reg := apiregistry.NewRegistrar(roles, "role")
	{
		...
		reg.GETProtected("/:id/actions", reg.Meta("获取角色功能权限").BindGroup("role").BindPermissionKey("system.role.assign_action").Build(), "system.role.assign_action", authzService.RequireAction, roleHandler.GetRoleActions)
		reg.PUTProtected("/:id/actions", reg.Meta("配置角色功能权限").BindGroup("role").BindPermissionKey("system.role.assign_action").Build(), "system.role.assign_action", authzService.RequireAction, roleHandler.SetRoleActions)
