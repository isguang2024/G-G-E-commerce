...
func (m *PermissionModule) RegisterRoutes(rg *gin.RouterGroup) {
	...
	reg := apiregistry.NewRegistrar(actions, "permission_key")
	{
		...
		reg.DELETEProtected("/:id/endpoints/:endpointId", reg.Meta("删除功能权限关联接口").BindPermissionKey("system.permission.manage").Build(), "system.permission.manage", authzService.RequireAction, handler.RemoveEndpoint)
