...
func (m *UserModule) RegisterRoutes(rg *gin.RouterGroup) {
	...
	reg := apiregistry.NewRegistrar(users, "user")
	{
		...
		reg.GETProtected("/:id/teams", reg.Meta("获取用户所在团队").BindGroup("user").BindPermissionKey("system.user.manage").Build(), "system.user.manage", authzService.RequireAction, userHandler.GetTeams)
