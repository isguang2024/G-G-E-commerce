...
func (m *UserModule) RegisterRoutes(rg *gin.RouterGroup) {
	...
	userHandler := NewUserHandler(userService, actionRepo, packageRepo, platformService, boundaryService, roleRepo, userRoleRepo, userPackageRepo, userHiddenMenuRepo, userActionRepo, menuRepo, refresher, m.logger)
	...
	reg := apiregistry.NewRegistrar(users, "user")
	{
		...
		reg.GET("/:id/actions", &apiregistry.RouteMeta{Summary: "获取用户权限例外审计", ResourceCode: "user", ActionCode: "assign_action"}, authzService.RequireAction("system.user.assign_action"), userHandler.GetActions)
		reg.PUT("/:id/actions", &apiregistry.RouteMeta{Summary: "配置用户权限例外审计", ResourceCode: "user", ActionCode: "assign_action"}, authzService.RequireAction("system.user.assign_action"), userHandler.SetActions)
