...
func DefaultAPIEndpointCategories() []APIEndpointCategorySeed {
	items := []APIEndpointCategorySeed{
		...
		{Code: "collaboration", Name: "协作空间", NameEn: "Collaboration", SortOrder: 120, Status: "normal"},
		...
		{Code: "site_config", Name: "参数管理", NameEn: "Parameter Management", SortOrder: 190, Status: "normal"},
...
func DefaultPermissionKeys() []PermissionKeySeed {
	items := []PermissionKeySeed{
		...
		newCanonicalPermissionKeySeed("workspace.manage", "工作空间管理", "允许查看和维护协作工作空间及其成员", "workspace", "personal"),
		newCanonicalPermissionKeySeed("collaboration.member.manage", "当前协作成员管理", "允许在当前协作上下文中查看和维护本协作空间成员", "collaboration_member", "collaboration"),
		newCanonicalPermissionKeySeed("collaboration.boundary.manage", "当前协作功能边界管理", "允许在当前协作上下文中查看和配置本协作空间的功能边界", "collaboration_boundary", "collaboration"),
		...
		newCanonicalPermissionKeySeed("feature_package.assign_workspace", "工作空间功能包开通", "允许为工作空间开通和配置功能包", "feature_package", "personal"),
		...
		newCanonicalPermissionKeySeed("collaboration.message.manage", "协作空间消息发送", "允许协作空间管理员向当前协作空间发送站内通知、消息和待办", "collaboration_message", "collaboration"),
		...
		newPermissionKeySeed("system.site_config", "view", "查询参数项（管理端）", "允许查询参数项列表"),
		newPermissionKeySeed("system.site_config", "manage", "管理参数项", "允许新增、更新、删除参数项和参数集合"),
...
func DefaultPermissionModuleGroups() []PermissionGroupSeed {
	return []PermissionGroupSeed{
		...
		},
		{
			...
			Code:        "site_config",
			Name:        "参数管理",
			NameEn:      "Parameter Management",
			Description: "参数项、参数集合与多作用域解析管理",
			Status:      "normal",
			SortOrder:   235,
			IsBuiltin:   true,
		},
	...
	items := []FeaturePackageSeed{
		{
			...
			Status:         "normal",
			SortOrder:      1,
			MenuNames:      []string{"System", "SystemAccess", "ActionPermission", "FeaturePackage", "SystemNavigation", "AppManage", "Menus", "PageManagement", "FastEnterManage", "MenuSpaceManage", "AccessTrace", "SystemIntegration", "ApiEndpoint", "MessageManage", "CollaborationRoot", "CollaborationManage"},
			PermissionKeys: []string{"system.permission.manage", "system.page.manage", "system.page.sync", "system.fast_enter.manage", "message.manage", "workspace.manage"},
		},
		{
		...
		},
		{
			...
			PermissionKeys: []string{"system.api_registry.view", "system.api_registry.sync", "feature_package.manage", "feature_package.assign_workspace"},
		},
		{
			...
			Status:         "normal",
			SortOrder:      4,
			MenuNames:      []string{"System", "SystemAccess", "Role", "User", "CollaborationRoot", "CollaborationMembers", "CollaborationRolesAndPermissions"},
		...
		},
		{
			...
			PackageType:    "base",
			Name:           "平台管理员内容与参数管理包",
			Description:    "聚合字典、媒体、参数管理、上传配置以及注册入口治理能力",
			WorkspaceScope: "all",
			ContextType:    "common",
			AppKeys:        []string{systemmodels.DefaultAppKey},
			IsBuiltin:      true,
		...
		{
			PackageKey:     "collaboration.member_admin",
			PackageType:    "base",
			Name:           "协作空间成员管理包",
			...
			Status:         "normal",
			SortOrder:      10,
			MenuNames:      []string{"CollaborationRoot", "CollaborationMembers", "CollaborationRolesAndPermissions", "CollaborationMessageManage"},
			PermissionKeys: []string{"collaboration.member.manage", "collaboration.boundary.manage", "collaboration.message.manage"},
		},
		{
...
func DefaultMenus() []MenuSeed {
	...
	metaCollaborationAccessOnly := usermodel.MetaJSON{"keepAlive": true}
	...
	items := []MenuSeed{
		...
		{Name: "CollaborationRoot", Kind: systemmodels.MenuKindDirectory, Path: "/collaboration", Component: "", Title: "协作空间", Icon: "ri:group-line", SortOrder: 6, Meta: metaCollaborationAccessOnly},
		...
		{Name: "SystemConfig", ParentName: "System", Kind: systemmodels.MenuKindDirectory, Path: "parameter", Component: "", Title: "参数管理", Icon: "ri:settings-3-line", SortOrder: 7, Meta: metaSuperAdmin},
		...
		{Name: "CollaborationManage", ParentName: "CollaborationRoot", Path: "workspaces", Component: "/collaboration/workspace", Title: "协作空间管理", SortOrder: 1, Meta: usermodel.MetaJSON{"keepAlive": true, "accessMode": "permission", "requiredAction": "workspace.manage"}},
		{Name: "CollaborationRolesAndPermissions", ParentName: "CollaborationRoot", Path: "roles", Component: "/system/collaboration-roles-permissions", Title: "协作空间角色与权限", SortOrder: 3, Meta: usermodel.MetaJSON{"keepAlive": true, "accessMode": "permission", "requiredAction": "workspace.manage"}},
		...
		{Name: "SiteConfig", ParentName: "SystemConfig", Path: "/system/parameter", Component: "/system/site-config", Title: "参数管理", SortOrder: 2, PermissionKey: "system.site_config.view", Meta: usermodel.MetaJSON{"roles": []interface{}{"R_SUPER"}, "permissions": []interface{}{"system.site_config.view"}, "keepAlive": true}},
		...
		{Name: "CollaborationMessageManage", ParentName: "CollaborationRoot", Path: "message", Component: "/collaboration/message", Title: "协作空间消息发送", SortOrder: 4, Meta: metaCollaborationAccessOnly},
		...
		{Name: "CollaborationMembers", ParentName: "CollaborationRoot", Path: "members", Component: "/collaboration/members", Title: "协作空间成员", SortOrder: 2, Meta: metaCollaborationAccessOnly},
	}
	for i := range items {
...
func DeprecatedDefaultMenuNames() []string {
	return []string{"PageAssociation", "CollaborationManagementRedirect", "Scope", "UserCenter"}
}
...
func DefaultRolePackageBindings() []RolePackageBindingSeed {
	items := []RolePackageBindingSeed{
		...
		{RoleCode: "collaboration_admin", PackageKey: "collaboration.member_admin"},
	}
	for i := range items {
...
func DefaultPages() []PageSeed {
	...
	items := []PageSeed{
		...
		{
			PageKey:           "collaboration.message.template.manage",
			Name:              "协作空间消息模板",
			RouteName:         "CollaborationMessageTemplateManage",
			RoutePath:         "/collaboration/message-template",
			Component:         "/collaboration/message-template",
			PageType:          "inner",
			Source:            "manual",
			ModuleKey:         "collaboration_message",
			SortOrder:         45,
			ParentMenuName:    "CollaborationMessageManage",
			DisplayGroupKey:   "display.system_pages",
			ActiveMenuPath:    "/collaboration/message",
			BreadcrumbMode:    "inherit_menu",
			AccessMode:        "permission",
			PermissionKey:     "collaboration.message.manage",
			InheritPermission: false,
			KeepAlive:         true,
			Status:            "normal",
			Meta:              usermodel.MetaJSON{},
		...
		{
			PageKey:           "collaboration.message.sender.manage",
			Name:              "协作空间发送人管理",
			RouteName:         "CollaborationMessageSenderManage",
			RoutePath:         "/collaboration/message-sender",
			Component:         "/collaboration/message-sender",
			PageType:          "inner",
			Source:            "manual",
			ModuleKey:         "collaboration_message",
			SortOrder:         44,
		...
		{
			PageKey:           "collaboration.message.recipient_group.manage",
			Name:              "协作空间接收组管理",
			RouteName:         "CollaborationMessageRecipientGroupManage",
			RoutePath:         "/collaboration/message-recipient-group",
			Component:         "/collaboration/message-recipient-group",
			PageType:          "inner",
			Source:            "manual",
			ModuleKey:         "collaboration_message",
			SortOrder:         43,
		...
		{
			PageKey:           "collaboration.message.record.manage",
			Name:              "协作空间发送记录",
			RouteName:         "CollaborationMessageRecordManage",
			RoutePath:         "/collaboration/message-record",
			Component:         "/collaboration/message-record",
			PageType:          "inner",
			Source:            "manual",
			ModuleKey:         "collaboration_message",
			SortOrder:         46,
		...
		{
			PageKey:           "collaboration.message.more",
			Name:              "协作空间更多入口",
			RouteName:         "CollaborationMore",
			RoutePath:         "/collaboration/more",
			Component:         "/collaboration/more",
			PageType:          "inner",
			Source:            "manual",
			ModuleKey:         "collaboration_message",
			SortOrder:         47,
...
func LegacyMenuBackedPages() []PageSeed {
	items := []PageSeed{
		...
		{
			MenuSpaceKey:      systemmodels.DefaultMenuSpaceKey,
			PageKey:           "collaboration.index",
			Name:              "Collaboration",
			RouteName:         "CollaborationIndex",
			RoutePath:         "/collaboration/workspaces",
			Component:         "/collaboration/workspace",
			PageType:          "inner",
			Source:            "manual",
			ModuleKey:         "workspace",
			SortOrder:         35,
			ParentMenuName:    "CollaborationManage",
			DisplayGroupKey:   "display.system_pages",
			ActiveMenuPath:    "/collaboration/workspaces",
			BreadcrumbMode:    "inherit_menu",
			AccessMode:        "permission",
			PermissionKey:     "workspace.manage",
			InheritPermission: false,
			KeepAlive:         true,
			Status:            "normal",
			Meta:              usermodel.MetaJSON{},
		...
		{
			MenuSpaceKey:      systemmodels.DefaultMenuSpaceKey,
			PageKey:           "collaboration.message.manage",
			Name:              "协作空间消息发送",
			RouteName:         "CollaborationMessageManage",
			RoutePath:         "/collaboration/message",
			Component:         "/collaboration/message",
			PageType:          "inner",
			Source:            "manual",
			ModuleKey:         "collaboration_message",
			SortOrder:         40,
...
func newCanonicalPermissionKeySeed(permissionKey, name, description, moduleCode, contextType string) PermissionKeySeed {
	mapping := permissionkey.FromKey(permissionKey)
	resolvedModuleCode := strings.TrimSpace(moduleCode)
	if resolvedModuleCode == "" {
		resolvedModuleCode = strings.TrimSpace(mapping.ResourceCode)
	}
	resolvedContextType := strings.TrimSpace(contextType)
	if resolvedContextType == "" {
		resolvedContextType = strings.TrimSpace(mapping.ContextType)
	}
	return PermissionKeySeed{
		...
		ModuleGroupCode:  resolvedModuleCode,
		FeatureGroupCode: "system",
		FeatureKind:      "system",
		Status:           "normal",
		IsBuiltin:        true,
