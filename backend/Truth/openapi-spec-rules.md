...
> 后端 API 契约真相源在 `backend/api/openapi/`。`backend/api/gen/`、权限种子和前端 `frontend/src/api/v5/` 都从这里派生。
...
生成链之外的完整闭环（model → spec → sub-handler → UI → verify）见 [api-openapi-flow.md](./api-openapi-flow.md)，不在本文重复。

## 改 `x-permission-key` 的协同清单

只要改了 `x-permission-key` 或 `x-access-mode`，以下四处必须同步：

1. `backend/api/openapi/domains/{domain}/paths.yaml`
...
1. 先看 [api-openapi-flow.md](./api-openapi-flow.md) 了解完整闭环
2. 再回到 `backend/api/openapi/` 改 spec
3. 最后对照 `internal/api/handlers/{domain}.go` 与前端调用收口实现（sub-handler 细节见 [backend-guide.md](./backend-guide.md)）
