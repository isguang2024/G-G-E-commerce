...
当前项目只维护 `frontend/` 这一条管理端前端主线。所有页面、壳层、组件、路由和样式规范都以该目录为唯一约束范围，不再并行发展第二套后台实现。
...
- 新页面、新分组和新壳层优先延续现有 `frontend/` 的信息架构，不用“新目录 + 新实现”绕开主线。
...
- 不用“临时新组件体系”或“第二套管理台皮肤”规避当前主线约束。
...
- 状态管理、上下文与页面元数据继续在 `frontend/` 主线内收口，不额外并行孵化第二套数据层。
