...
- 使用中文沟通。
- 仓库当前只保留一条有效前端主线：`frontend/`。
- 所有管理端页面、壳层、路由、状态与样式演进默认都在 `frontend/` 内完成，不额外新开第二条管理端工程或重写线。
...
- 单条前端主线内部不要混用多套后台 UI 体系，不并行孵化第二套后台设计语言。
- 当前项目的前端规范、页面结构和管理台实现都以 `frontend/` 为唯一落点；其它目录不再承载新的管理端主线职责。
