<template>
  <ElDrawer
    v-model="visible"
    :title="`协作空间菜单边界 - ${props.collaborationName}`"
    size="960px"
    destroy-on-close
    class="collaboration-menu-boundary-dialog config-drawer"
    direction="rtl"
  >
...
<script setup lang="ts">
  ...
  import {
    fetchGetCollaborationMenus,
    fetchGetCollaborationMenuOrigins,
    fetchSetCollaborationMenus
  ...
  async function loadData() {
    ...
    try {
      const [allMenus, currentRes, packageRes, originRes] = await Promise.all([
        ...
        fetchGetCollaborationMenus(props.collaborationId, currentAppKey.value),
        fetchGetCollaborationWorkspaceFeaturePackages(
  ...
  async function handleSave() {
    ...
    saving.value = true
    try {
      await fetchSetCollaborationMenus(
        props.collaborationId,
