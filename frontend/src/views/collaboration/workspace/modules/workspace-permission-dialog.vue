<template>
  <ElDrawer
    v-model="visible"
    :title="`协作空间边界 - ${props.collaborationName}`"
    size="960px"
    destroy-on-close
    class="collaboration-permission-dialog config-drawer"
    direction="rtl"
  >
...
<script setup lang="ts">
  ...
  import {
    fetchGetCollaborationActions,
    fetchGetCollaborationActionOrigins,
    fetchSetCollaborationActions
  ...
  const summaryItems = computed(() => [
    { label: '协作空间', value: props.collaborationName || '-' },
  ...
  async function loadData() {
    if (!props.collaborationId || !currentAppKey.value) {
    ...
    try {
      const [actionsRes, currentRes, packageRes, originsRes] = await Promise.all([
        ...
        fetchGetCollaborationActions(props.collaborationId, currentAppKey.value),
        fetchGetCollaborationWorkspaceFeaturePackages(
  ...
  async function handleSave() {
    ...
    saving.value = true
    try {
      await fetchSetCollaborationActions(
        props.collaborationId,
