...
<script setup lang="ts">
  ...
  const menuTitle = computed(
    () => props.menuData?.meta?.title || props.menuData?.name || '当前菜单'
  )
    ...
    try {
      ...
      const initialActions = Array.from(
        new Set(
          [
            meta.requiredAction,
            ...(Array.isArray(meta.requiredActions) ? meta.requiredActions : [])
          ]
  ...
  async function loadPermissionActions() {
    ...
    if (!permissionActionsPromise) {
      permissionActionsPromise = fetchGetPermissionActionOptions({ status: 'normal' })
  ...
  function normalizeRequiredActions(
    ...
    availableActions.forEach((item) => {
      const key = buildScopedActionKey(
        item.permissionKey || `${item.resourceCode}:${item.actionCode}`
      )
