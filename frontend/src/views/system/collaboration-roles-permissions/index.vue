<template>
  <div class="art-full-height">
    <AdminWorkspaceHero
      ...
      :metrics="heroMetrics"
    >
      <div class="collaboration-role-hero-actions">
        <AppKeySelect
          v-model="selectedAppKey"
          placeholder="选择 App"
          class="collaboration-role-app-select"
          @change="handleManagedAppChange"
        />
        <ElButton
          v-if="hasAction('collaboration.boundary.manage')"
          type="primary"
          @click="openAddDialog"
          >新增协作空间角色</ElButton
        >
...
<script setup lang="ts">
  ...
  import {
    fetchDeleteMyCollaborationRole,
    fetchGetMyCollaborationBoundaryRoles
  ...
  const {
    ...
    core: {
      apiFn: async () => {
        ...
        if (!hasAction('collaboration.boundary.manage')) {
          return {
            records: [],
            total: 0,
            current: 1,
        ...
        const list = (await fetchGetMyCollaborationBoundaryRoles(
          targetAppKey.value
        )) as RoleListItem[]
        return {
          records: list,
      ...
      columnsFactory: () => [
        ...
        { prop: 'createTime', label: '创建时间', width: 170 },
        {
          ...
          formatter: (row: RoleListItem) => {
            ...
            if (!row.isGlobal && hasAction('collaboration.boundary.manage')) {
  ...
  async function buttonMoreClick(item: ButtonMoreItem, row: RoleListItem) {
    ...
    if (item.key === 'delete') {
      ...
      await fetchDeleteMyCollaborationRole(row.roleId)
      onSuccess()
    }
  }
...
<style scoped lang="scss">
  .collaboration-role-hero-actions {
    display: flex;
    flex-wrap: wrap;
    gap: 12px;
  }

  .collaboration-role-app-select {
    width: 240px;
  }
