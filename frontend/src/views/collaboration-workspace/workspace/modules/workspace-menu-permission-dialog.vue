<template>
  <ElDrawer
    v-model="visible"
    :title="`协作空间菜单边界 - ${props.collaborationWorkspaceName}`"
    size="960px"
    destroy-on-close
    class="collaboration-workspace-menu-boundary-dialog config-drawer"
    direction="rtl"
  >
...
<script setup lang="ts">
  ...
  import {
    ...
    fetchSetCollaborationWorkspaceMenus
  ...
  async function loadData() {
    ...
    try {
      const [allMenus, currentRes, packageRes, originRes] = await Promise.all([
        ...
        fetchGetCollaborationWorkspaceMenus(props.collaborationWorkspaceId, currentAppKey.value),
        fetchGetCollaborationWorkspaceFeaturePackages(
  ...
  async function handleSave() {
    ...
    saving.value = true
    try {
      await fetchSetCollaborationWorkspaceMenus(
        props.collaborationWorkspaceId,
