<template>
  <div
    ...
    class="collaboration-switcher"
    :class="{ 'max-md:!hidden': !compact, 'collaboration-switcher-compact': compact }"
  >
    <div v-if="compact" class="collaboration-label">{{ currentContextLabel }}</div>
    <ElSelect
      :model-value="selectedValue"
      class="collaboration-select"
      size="default"
      placeholder="切换工作空间"
      :loading="loading"
      @change="handleChange"
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'ArtCollaborationSwitcher' })
  ...
  const { collaborationList } = storeToRefs(collaborationStore)
  const {
    workspaceList,
    personalWorkspace,
  ...
  const buildWorkspaceLabel = (workspace: Api.SystemManage.WorkspaceItem) => {
    ...
    const matchedCollaborationWorkspace = collaborationList.value.find(
      (item) => item.workspaceId === workspace.id
    )
    const roleLabel =
      matchedCollaborationWorkspace?.currentRoleCode === 'collaboration_admin'
        ? '管理员视图'
        : '成员视图'
...
<style scoped lang="scss">
  ...
  .collaboration-switcher-compact {
    width: 100%;
    min-width: 0;
    margin-right: 0;
  }
  ...
  .collaboration-switcher-compact :deep(.el-select__wrapper) {
    min-height: 40px;
    background:
