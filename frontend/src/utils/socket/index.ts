...
export default class WebSocketClient {
  ...
  send(data: string | ArrayBufferLike | Blob | ArrayBufferView, immediate: boolean = false): void {
    ...
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      this.debug('WebSocket未连接，消息已加入队列等待发送')
  ...
  private flushMessageQueue(): void {
    if (this.messageQueue.length > 0 && this.ws?.readyState === WebSocket.OPEN) {
      this.debug(`发送队列中的${this.messageQueue.length}条消息`)
  ...
  private handleOpen(event: Event): void {
    this.debug('WebSocket连接成功', event)
  ...
  private handleMessage(event: MessageEvent): void {
    this.debug('收到WebSocket消息:', event)
    this.resetHeartbeat()
  ...
  private reconnect(): void {
    ...
    this.reconnectTimer = setTimeout(() => {
      this.debug(`尝试重新连接WebSocket（第${this.reconnectAttempts}次）`)
      this.init()
      this.stopReconnect = false
    }, delay)
  }
