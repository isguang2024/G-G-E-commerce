...
import { createUnifiedV5HttpError, unwrapV5Response } from '@/utils/http/v5'
...
// @compat-status: transition v5 schema 仍通过这里桥接回旧 Api.SystemManage.* 契约，当前不能直接删除。
...
type V5PermissionBatchTemplatePayload =
  components['schemas']['PermissionActionBatchTemplatePayload']
...
type V5PermissionActionConsumerDetailsLike =
  components['schemas']['PermissionActionConsumersResponse']
...
export function createV5HttpError(error: any, response?: Response) {
  return createUnifiedV5HttpError(error, response)
}
...
export async function unwrap<T>(
  ...
  return unwrapV5Response(promise)
}
...
export function deriveContextType(permissionKey?: string, moduleCode?: string) {
  ...
  if (key.startsWith('personal.') || module === 'personal' || module === 'personal_workspace') {
    return 'personal'
  }
  if (
...
export function normalizePermissionAction(
  item: V5PermissionActionLike
): Api.SystemManage.PermissionActionItem {
...
export function normalizePermissionAuditSummary(
  ...
  return {
    ...
    crossContextMirrorCount: Number(item?.cross_context_mirror_count ?? 0),
    suspectedDuplicateCount: Number(item?.suspected_duplicate_count ?? 0)
  }
}
...
export function normalizeApiEndpoint(item: any): Api.SystemManage.APIEndpointItem {
  ...
  const permissionKeys = Array.isArray(permissionKeysRaw)
    ...
    : []
  const permissionKey = normalizePermissionKey(item?.permission_key) || permissionKeys[0] || ''
  return {
    id: item?.id || '',
    code: item?.code || '',
    method: item?.method || '',
    ...
    authMode: item?.auth_mode || item?.access_mode || (permissionKey ? 'permission' : 'jwt'),
...
export function normalizeFeaturePackage(item: any): Api.SystemManage.FeaturePackageItem {
  ...
  return {
    ...
    collaborationWorkspaceCount: item?.workspace_count ?? item?.collaboration_workspace_count ?? 0,
    status: item?.status || 'normal',
    sortOrder: item?.sort_order ?? 0,
    createdAt: item?.created_at || '',
    updatedAt: item?.updated_at || ''
...
export function normalizeCollaboration(
  item: any
...
export function normalizePageItem(item: V5PageLike | undefined): Api.SystemManage.PageItem {
  ...
  const remoteBinding = {
    manifestUrl:
      `${rawRemoteBinding?.manifest_url || meta?.manifest_url || meta?.manifestUrl || ''}`.trim(),
    remoteAppKey:
      `${rawRemoteBinding?.remote_app_key || meta?.remote_app_key || meta?.remoteAppKey || ''}`.trim(),
    remotePageKey:
      `${rawRemoteBinding?.remote_page_key || meta?.remote_page_key || meta?.remotePageKey || ''}`.trim(),
    remoteEntryUrl:
      `${rawRemoteBinding?.remote_entry_url || meta?.remote_entry_url || meta?.remoteEntryUrl || ''}`.trim(),
    remoteRoutePath:
      `${rawRemoteBinding?.remote_route_path || meta?.remote_route_path || meta?.remoteRoutePath || ''}`.trim(),
    remoteModule:
      `${rawRemoteBinding?.remote_module || meta?.remote_module || meta?.remoteModule || ''}`.trim(),
    remoteModuleName:
      `${rawRemoteBinding?.remote_module_name || meta?.remote_module_name || meta?.remoteModuleName || ''}`.trim(),
    remoteUrl:
      `${rawRemoteBinding?.remote_url || meta?.remote_url || meta?.remoteUrl || ''}`.trim(),
    runtimeVersion:
      `${rawRemoteBinding?.runtime_version || meta?.runtime_version || meta?.runtimeVersion || meta?.version || ''}`.trim(),
    healthCheckUrl:
      `${rawRemoteBinding?.health_check_url || meta?.health_check_url || meta?.healthCheckUrl || ''}`.trim()
  }
...
export function normalizeMenuSpace(item: any): Api.SystemManage.MenuSpaceItem {
  ...
  const rawAccessMode = item?.access_mode || item?.meta?.access_mode || 'all'
...
export function normalizeMenuSpaceHostBinding(
  item: any
): Api.SystemManage.MenuSpaceHostBindingItem {
  const meta = item?.meta || {}
  return {
    id: item?.id || '',
    appKey: item?.app_key || '',
    ...
    routePrefix: `${item?.route_prefix || meta?.route_prefix || meta?.routePrefix || ''}`.trim(),
    authMode:
      `${item?.auth_mode || meta?.auth_mode || meta?.authMode || 'inherit_host'}`.trim() ||
      'inherit_host',
    loginHost: `${item?.login_host || meta?.login_host || meta?.loginHost || ''}`.trim(),
    callbackHost:
...
export function normalizeApp(item: any): Api.SystemManage.AppItem {
  ...
  return {
    ...
    manifestUrl:
      `${item?.manifest_url || item?.meta?.manifest_url || item?.meta?.manifestUrl || ''}`.trim(),
    runtimeVersion:
      `${item?.runtime_version || item?.meta?.runtime_version || item?.meta?.runtimeVersion || item?.meta?.version || ''}`.trim(),
    ...
    menuSpaceCount: Number(item?.menu_space_count ?? item?.space_count ?? 0),
    menuCount: Number(item?.menu_count ?? 0),
    pageCount: Number(item?.page_count ?? 0),
    meta: item?.meta || {},
    createdAt: item?.created_at || '',
...
export function normalizeMenuSpaceEntryBinding(
  item: any
): Api.SystemManage.MenuSpaceEntryBindingItem {
  return {
    id: item?.id || '',
    appKey: item?.app_key || '',
    appName: item?.app_name || '',
...
export function normalizeRefreshStats(
  item: V5RefreshStatsLike | undefined
): Api.SystemManage.RefreshStats {
  const legacy = item as any
  return {
    requestedPackageCount: Number(item?.requested_package_count ?? 0),
    impactedPackageCount: Number(item?.impacted_package_count ?? 0),
    roleCount: Number(item?.role_count ?? 0),
    collaborationWorkspaceCount: Number(item?.workspace_count ?? legacy?.collaboration_workspace_count ?? 0),
...
export function normalizeUnregisteredApiScanConfig(
  item: any
...
export function normalizePageAccessTraceResult(
  ...
  return {
    userId: item?.user_id || '',
    collaborationWorkspaceId: item?.workspace_id || legacy?.collaboration_workspace_id || '',
    menuSpaceKey: item?.menu_space_key || '',
...
export function normalizeRuntimeNavigationManifest(
  item: any
...
export function normalizeUnregisteredApiRoute(
  item: any
): Api.SystemManage.APIUnregisteredRouteItem {
  return {
    method: item?.method || '',
    path: item?.path || '',
...
export function normalizeUserPermissionDiagnosisResponse(
  ...
  const diagnosis = item?.diagnosis
    ? {
        ...
        blockedByCollaborationWorkspace: Boolean(
          item.diagnosis?.blocked_by_collaboration || legacy?.diagnosis?.blocked_by_collaboration_workspace
        ),
  ...
  const context = {
    ...
    collaborationWorkspaceId:
      item?.context?.current_workspace_id || legacy?.context?.current_collaboration_workspace_id || '',
    collaborationWorkspaceName:
      item?.context?.current_workspace_name || legacy?.context?.current_collaboration_workspace_name || ''
  ...
  return {
    ...
    collaborationWorkspaceMember: (item?.collaboration_member || legacy?.collaboration_workspace_member)
      ? {
          id: item?.collaboration_member?.id || legacy?.collaboration_workspace_member?.id || '',
          collaborationWorkspaceId:
            item?.collaboration_member?.workspace_id ||
            legacy?.collaboration_workspace_member?.collaboration_workspace_id ||
            '',
          userId: item?.collaboration_member?.user_id || legacy?.collaboration_workspace_member?.user_id || '',
          roleCode:
            item?.collaboration_member?.role_code || legacy?.collaboration_workspace_member?.role_code || '',
          status: item?.collaboration_member?.status || legacy?.collaboration_workspace_member?.status || '',
          matched: Boolean(item?.collaboration_member?.matched ?? legacy?.collaboration_workspace_member?.matched)
        }
      : null,
    collaborationWorkspacePackages: normalizePackages(
      item?.workspace_packages || legacy?.collaboration_workspace_packages
    ),
    diagnosis,
